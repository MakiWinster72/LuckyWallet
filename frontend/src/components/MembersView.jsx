...
function MemberFinanceChart({ finance }) {
  ...
  return (
    <div className="member-finance-chart">
      <div className="member-chart-legend">
      ...
      <svg
        viewBox="0 0 420 145"
        role="img"
        aria-label="近六个月个人支出与应收收入曲线"
      >
        <path
          d="M10 125H410M10 72.5H410M10 20H410"
          className="member-chart-grid"
        />
        <path
          d={buildSmoothSvgPath(expensePoints)}
          className="member-expense-curve"
        />
        <path
          d={buildSmoothSvgPath(incomePoints)}
          className="member-income-curve"
        />
        {expensePoints.map((point, index) => (
          <circle
            className="member-expense-point"
            key={`expense-${finance.series[index].month}`}
            cx={point.x}
            cy={point.y}
            r="3"
          >
            <title>
              {finance.series[index].label}支出{" "}
              {formatMoney(finance.series[index].expense)}
            </title>
          </circle>
        ))}
        {incomePoints.map((point, index) => (
          <circle
            className="member-income-point"
            key={`income-${finance.series[index].month}`}
            cx={point.x}
            cy={point.y}
            r="3"
          >
            <title>
              {finance.series[index].label}应收{" "}
              {formatMoney(finance.series[index].income)}
            </title>
          </circle>
        ))}
      </svg>
      <div className="member-chart-axis">
        {finance.series.map((item) => (
...
export function MembersView({ members, bills, currentMemberId }) {
  ...
  const stats = useMemo(
    () => buildMemberStats(members, bills),
    [members, bills],
  );
  ...
  const selectedFinance = useMemo(
    () =>
      selected
        ? summarizeMemberFinance(bills, selected.member.id, referenceDate)
        : null,
  ...
  return (
    <section className="members-view" aria-labelledby="members-title">
      <header className="members-heading">
        <div>
          <h2 id="members-title">共同成员</h2>
          <p>看看每个人这个月的参与和垫付情况。</p>
        </div>
      ...
      <div className="member-summary">
        ...
        <article>
          <span>待收回金额</span>
          <strong>{formatMoney(summary.positive)}</strong>
          <p>结算后即可清零</p>
        </article>
      </div>
          ...
          >
            <header>
              ...
              <div>
                ...
                <span>最近参与 {item.lastActive ?? "暂无记录"}</span>
              </div>
              <span className="member-card-more" aria-hidden="true">
                •••
              </span>
            </header>
            <dl>
              <div>
                <dt>本月垫付</dt>
                <dd>{formatMoney(item.paid)}</dd>
              </div>
              <div>
                <dt>参与账单</dt>
                <dd>{item.billCount} 笔</dd>
              </div>
            </dl>
        ...
        >
          ...
          >
            ...
            <dl>
              <div>
                <dt>累计垫付</dt>
                <dd>{formatMoney(selected.paid)}</dd>
              </div>
              <div>
                <dt>个人分摊</dt>
                <dd>{formatMoney(selected.share)}</dd>
              </div>
              <div>
                <dt>参与账单</dt>
                <dd>{selected.billCount} 笔</dd>
              </div>
            </dl>
            <div
              className={
                selected.balance >= 0
                  ? "member-balance is-credit"
                  : "member-balance is-debit"
              }
            >
            ...
            <section
              ...
              aria-labelledby="member-finance-title"
            >
              <header>
                <div>
                  <span className="overline">CASH FLOW</span>
                  <h3 id="member-finance-title">收支统计</h3>
                </div>
                <small>收入按代付应收口径</small>
              </header>
              <div className="member-finance-kpis">
                <article>
                  <span>个人支出</span>
                  <strong>{formatMoney(selectedFinance.expense)}</strong>
                </article>
                <article>
                  <span>收入（应收）</span>
                  <strong>{formatMoney(selectedFinance.income)}</strong>
                </article>
                <article>
                  <span>收支差额</span>
                  <strong
                    className={
                      selectedFinance.net >= 0 ? "is-positive" : "is-negative"
                    }
                  >
                    {formatMoney(selectedFinance.net)}
                  </strong>
                </article>
              </div>
