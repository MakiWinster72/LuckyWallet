    ...
    <div className="bill-register panel">
      <header>
        ...
        <label className="bill-register-search">
          ...
          <input name="bill-register-search" type="search" autoComplete="off" value={query}
            onChange={(event) => onQuery(event.target.value)} placeholder="搜索账单、分类或付款人…" />
          {query ? <button type="button" onClick={() => onQuery("")} aria-label="清除账单搜索">清除</button> : null}
        </label>
        <div><span>筛选结果合计</span><strong>{formatMoney(visibleTotal)}</strong></div>
      </header>
      <p className="bill-result-status" aria-live="polite">
        {query ? `“${query}”找到 ${bills.length} 笔账单` : `当前显示 ${bills.length} 笔账单`}
      </p>
      {dateError ? (
        ...
        <>
          <div className="bill-table-scroll">
            <table>
              ...
              <tbody>{pagination.items.map((bill) => {
      ...
      ) : <div className="bill-empty"><span>{query ? `没有找到“${query}”` : "没有符合条件的账单"}</span><p>{query ? "尝试搜索其他账单名称、分类或付款人。" : "调整筛选条件，或新增本账期的第一笔消费。"}</p>{query ? <button type="button" onClick={() => onQuery("")}>清除搜索</button> : <button type="button" onClick={onAdd}>新增账单</button>}</div>}
    </div>
  );
}
  ...
  return (
    <section className="bills-view" aria-labelledby="bills-title">
      <header className="bills-heading">
        <div><span className="overline">ALL ENTRIES</span><h2 id="bills-title">全部账单</h2><p>筛选、核对并查看每一笔共同消费。</p></div>
        <button className="add-button" type="button" onClick={onAdd}><AppIcon name="plus" size={18} />新增账单</button>
      </header>
