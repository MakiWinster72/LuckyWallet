...
LuckyWallet 是一个面向团队的共享记账工具。它不仅记录消费金额，还会记录**谁先付款、谁参与消费、每个人应承担多少**，从而生成清晰的收支统计和结算方案。
...
| Claude Code 助手 | 在项目上下文中提问；权限模式随账号角色变化 |
...
用户可以上传 `JPEG`、`PNG` 或 `WebP` 格式的头像，文件大小不能超过 5 MB。昵称由管理员在“用户管理”页面维护，修改后会同步到成员、账单和统计页面。
...
登录后，点击页面右下角的 Claude Code 图标可打开项目助手。它通过当前项目上下文回答问题，对话历史按用户保存在当前浏览器中。

账号角色决定助手权限：

- 普通用户使用 `plan` 模式，只分析问题和生成方案，不执行命令；
- 管理员使用 `auto` 模式，可能执行命令并修改项目文件。

::: danger 管理员模式
Claude Code 的管理员模式具有较高执行权限。不要向不可信账号授予管理员角色，发送指令前也应确认操作范围。该功能需要服务器安装并登录 Claude Code CLI；未配置时不影响其他记账功能。
:::
...
```bash
cd sql
mysql -u root -p < 004_reset_database.sql
```
